...
public class Evaluator {
  ...
  public static float evalPoly2(float[] A, int v) {
    ...
    // was j = A.length - 1;
    // j = -1 because the invariant will hold true. When you plug in j to the invariant
    //    the val = 0 (as stated above with float val = 0). Then 1 is added to j within
    //    the loop so that the invariant still holds with the new val reassignment.
