...
public class Natural {
  ...
  public static int leadingDigit(int[] digits) {
    ...
    // At this point in the code, we know that D[i+1] ... D[n-1] are all zero and i = i - 1.
    // This implies the postcondition below, since D[i+1]... D[n-1] are all zero in both and
    // i = 0 since the condition in the while loop is i does not equal 0 and we have exited the loop
    // so i = 0.
  ...
  public Natural toBase(int base) {
    ...
    while (j != this.digits.length - 1) {  // TODO: Replace the condition here with a suitable one.
      ...
      i = i - 1;
      j = j + 1;
      //r = r.times(base).plus(this.digits[i]);
      r = r.times(b).plus(new Natural(base, this.digits[i]));
    }

    // TODO: Explain why the postcondition holds at the end of this code.
    // The postcondition holds at the end of this code because the r is updated each loop
    // through the array so that it has the correct value. As i is updated through each time
    // in the loop and j is updated til j reaches n - 1, i is 0, therefore r = (base, D[0] + D[1] b +
    // D[2] b^2 + ... + D[n-1] b^{n-1}) which is equivalent to r = (base, this.value()).
  ...
  public String toString() {
    ...
    while (i != -1) {
      char digitBase = BaseDigits.digitToChar(this.digits[i], this.base);
      buf.append(digitBase);
      i = i - 1;
    }


    // At this point in the code, we know that buf = ch(D[n-1]), ch(D[n-2]), ..., ch(D[i+1])
    // and i = -1.
    // This implies the postcondition below, since if we substitute i = -1 into the invariant we get
    // buf = ch(D[n-1]), ch(D[n-2]), ..., ch(D[0]) (which is the post condition).
  ...
  public Natural plus(Natural other) {
    ...
    // At each index of the array digits, that value is the sum of the values at that same index in arrays A and B.
    ...
    // The values in this.digits and other.digits are always less than the base, therefore
    // if the base is multiplied by two, we always know that the sum of this.digits and other.digits
    // (newDigits D[]) will always be less than 2b.

    // TODO: Explain why the invariant of the loop below holds initially (no code needed).
    // The invariant below holds initially because D[0] is the sum of this.digits and other.digits
    // denoted A[] and B[]. It adds only til the all of the values of other.digits have been added.
    // This holds true in the invariant because the sums are added until i is reached (which is the length of
    // other.digits). If there are remaining values in this.digits then 0 is added to them since there are
    // no more indexes in other.digits.

    // TODO: Summary comment here
    // Within this loop, each index of the new array contains the sum of this.digits and other.digits at that
    //  index. Once the length of other.digits has been reached, the values added to the remaining
    //  values from this.digits is 0.
    ...
    // The element at each index of newDigits (denoted D[i] is the sum of this.digits and
    // other.digits (and the remainder of this.digits if other.digits.length is less than
    // this.digits.length), therefore this.value plus other.getValue is equal to the sum of
    // each of the elements in newDigits because this.getValue and other.getValue returns
    // the total value from the elements in the array with respect to its base, and those
    // are added together.
    ...
    // still represent the value this.getValue + other.getValue.
    ...
    // (1) The post condition holds because the elements of newDigits accurately represent the sum of
    // the arrays this.digits and other.digits multiplied by its designated base to a power based
    // off of what index the element is. (2) This also satisfies the precondition of the constructor
    // because after doing arithmetic in the previous while loop, all the digits are between 0 and
    // base - 1. Also, from the rep invariant, the bases are between 2 and 36 inclusively.
